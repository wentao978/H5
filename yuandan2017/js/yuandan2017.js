	...
	$.ajax({
		...
		success: function(data){
			...
			if(data.st == "2"){  //活动正常
				...
				$(".chooseCon").click(function(){
					if($(".chooseName").val().trim() == ""){
						$('.chooseName').focus();
						toast("请输入姓名",2600);
					}else{
						$('.load').show();	
						$.ajax({
							url: '/active/yuandan2017/choose.php',
							type: 'POST',
							dataType: 'json',
							timeout:15000,
							data: {
							...
							success: function(data,status,code){
								$("#toast").hide();
								$(".chooseName").val('');
								$("#content").removeClass("one").addClass("two");
								$(".qWrap").find("img").attr("src","images/q_"+data.number+".png");
								$('.load').hide();
							},
				...
				$(".replayWrap").click(function(){
					$("#content").removeClass("two").addClass("one");	
				});
				...
				$(".hbWrap").click(function(){
					...
					if(resNumber == 1){ //这个值为1才可以抽奖
						...
						$.ajax({
							url: '/active/yuandan2017/click.php',
							type: 'POST',
							dataType: 'json',
							timeout:60000,
							data:{
						...
						if(resLevel == 0){//这个值为0，说明抽过了并且没中奖
							...
							switch (resLevel){
								...
								case 1:   //手机地址									
									...
									$(".addressInfo").find(".prizeImg").attr("src","images/phone.png");
	...
	$(".phoneCon").click(function(){
		if($('.phoneNum').val() == ''){
			...
			if(!rePhone.test($(".phoneNum").val())){
				...
				$('.load').show();	
				$.ajax({
					url: '/active/yuandan2017/save.php',
					type: 'POST',
					dataType: 'json',
					timeout:15000,
					data: {
