	...
	$.ajax({
		...
		success: function(data){
			...
			if(data.st == "2"){  //活动正常
				...
				if(data.musicnumber != "3"){
					$(".tjinfo").html("您已推荐"+(3-parseInt(data.musicnumber))+"次歌曲，剩余"+data.musicnumber+"次额外摇奖机会！");
				}
						...
						if(parseInt(resNumber)){
							...
							setTimeout(function(){
								...
								switch (resLevel){
									...
									case "1":  //M6手机													
										$("#mask,.lingqu").show();
										$(".lingqu").find(".prize").attr("src","images/p_"+resLevel+".png");
